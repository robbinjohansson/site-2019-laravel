<header class="bg-white text-xs py-2 mb-12">
    <div class="container">
        <div class="md:flex md:flex-wrap -mx-4 md:justify-center">
            <div class="w-full md:w-3/4 lg:w-1/2 px-4 flex flex-wrap items-center justify-between">
                <div class="flex-grow-0">
                    <a href="/">Robbin Johansson</a>
                </div>
                <nav class="flex-1 text-right">
                    <a href="{{ route('about') }}" class="ml-2 md:ml-3">About</a>
                    <a href="{{ route('posts.archive') }}" class="ml-2 md:ml-3">Archive</a>
                    <a href="https://github.com/robbinworks" target="_BLANK" rel="noopener" class="ml-2 md:ml-3">Github</a>
                </nav>
            </div>
        </div>
    </div>
</header>
